...
namespace Dibuixos.Draws.Text
{
    ...
    public class TextInSpiralCenterFontQuickFont : GameWindow
    {
        ...
        private double mTimeFinishTextAll = 7;
        ...
        private string[][] mTextInitial = new[]
        {
            new[] { "Dibuixos", "Matematics", "mathematical-drawings for C#" },
            new[]{ "Mono OpenTK","Copyright 1995-2015-2016-2017","Oscar Hernández Bañó" }
        ...
        private void CreateCarsTexto()
        {
            while (!mTextos.MoveNext())
            {
                mTextos = GetTextos().GetEnumerator();
            }

            CreateCarsTexto(mTextos.Current);
        }

        private IEnumerable<string[]> GetTextos()
        {
            foreach (var pText in mTextInitial)
            {
                yield return pText;
            }

            mFortunes = mFortunes.Shuffle().ToArray();

            foreach (var pText in mFortunes)
            {
                var pTextInScreen = new List<string>();

                foreach (var pTextLine in pText)
                {
                    var pLen = pTextLine.Length;
                    var i = 0;

                    while (pLen > 0)
                    {
                        var pLen2 = Math.Min(pLen, 15);

                        pTextInScreen.Add(pTextLine.Substring(i, pLen2));
                        i += pLen2;
                        pLen -= pLen2;
                        if (pTextInScreen.Count >= 3)
                        {
                            yield return pTextInScreen.ToArray();
                            pTextInScreen.Clear();
                        }
                    }
                }
                if (pTextInScreen.Count > 0)
                {
                    yield return pTextInScreen.ToArray();
                    pTextInScreen.Clear();
                }
